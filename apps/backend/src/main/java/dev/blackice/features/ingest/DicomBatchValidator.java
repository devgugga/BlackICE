...
@ApplicationScoped
public class DicomBatchValidator {
            ...
            try (DicomInputStream in = new DicomInputStream(upload.path().toFile())) {
                ...
                if (ds == null) {
                    issues.add(new DicomValidationIssue(
                        ...
                        "Arquivo DICOM não contém dataset"
                    ));
                    continue;
                }
                ...
                issues.add(new DicomValidationIssue(
                    upload.filename(),
                    DicomValidationIssue.Code.MALFORMED_DICOM,
                    e.getMessage() != null ? e.getMessage() : "Erro ao processar arquivo DICOM"
                ));
            }
        }
                ...
                if (allIdentical) {
                    ...
                    for (int i = 1; i < group.size(); i++) {
                        ...
                        issues.add(new DicomValidationIssue(
                            ...
                            "Arquivo duplicado idêntico com SOPInstanceUID: " + entry.getKey()
                        ));
                    }
                } else {
                    for (ValidatedDicom collision : group) {
                        issues.add(new DicomValidationIssue(
                            ...
                            "Conflito de SOPInstanceUID com conteúdo diferente: " + entry.getKey()
                        ));
                    }
                }
            }
    ...
    private String requireUid(Attributes ds, int tag, DicomValidationIssue.Code code) {
        ...
        if (val == null || val.isBlank()) {
            throw new ValidationException(code, "UID obrigatório ausente: " + code);
        }
        return val;
    }
