...
foreach ($attempt in 1..30) {
  ...
  if (
    ...
    $location -match '^https://localhost:8843/realms/dcm4chee/protocol/openid-connect/auth\?'
  ) {
    $isReady = $true
    break
  }
...
2. aguardar, sem seguir redirects, `/ = 200`, `/api/me = 401` sem sessão e `/api/login = 302` com `Location` para `https://localhost:8843/realms/dcm4chee/protocol/openid-connect/auth?...`;
